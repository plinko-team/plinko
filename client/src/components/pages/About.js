      ...
      <div className="main-content">
        ...
        <Carousel swipeable={true} emulateTouch={true} transitionTime={0} showThumbs={false} infiniteLoop={true} showStatus={false}>
            <div>
                ...
                <p classNameName="legend">Legend 1</p>
            </div>
            <div>
                <img src="https://i.imgur.com/is6kNv7.png" />
                <p classNameName="legend">Legend 1</p>
...
<div id="doc" className="markdown-body container-fluid" style="position: relative;">
  ...
  <blockquote>
    <p>Before we take a stab at binary serialization, there’s a quick optimization we can make. You probably noticed that the values in the chip above include floating point numbers, some with many digits after the decimal. If we leave them as-is, we’ll need to represent every single digit in binary, even though there’s little or no perceivable difference between a chip rendered at x = 752.34235235235 (47 bits when serialized to binary) and a chip rendered at x = 752 (10 bits when serialized). To save ourselves some bits, we should <strong>quantize</strong> all our values before we serialize them. Here, quantization just means rounding to integers. (We do need to retain some decimals on the angle property for accuracy, so for that value we’ll round to three decimal points and multiply by 1000.)</p>
  </blockquote>
  ...
  <p>
    <img src="https://i.imgur.com/OFRLlrv.png" alt="Deserializing a Snapshot" /><br>
  ...
  <p>
    <img src="https://i.imgur.com/OaAMwvE.png" alt="Deserializing a Chip" /><br>
  ...
  <p><img src="https://s1.gifyu.com/images/mario-cs-prediction.gif" alt="Client side prediction" /></p>
  ...
  <p><img src="https://s1.gifyu.com/images/mario-wifi.gif" alt="Extrapolation" /></p>
  ...
  <p><img src="https://i.imgur.com/9DcONlU.png" alt="Smart Clients and Server" /></p>
  ...
  </ul><p>Due to network latency, the client and server receive information about what is happening on the other “in the past”, which then needs to be reconciled. Let’s take a look at how the system works as a whole before focusing on the individual parts:</p><blockquote>
  <p><img src="https://i.imgur.com/JVx8Ee4.png" alt="Extrapolation lifecycle" /><br>
    Figure 4.3A: Round-trip lifecycle of an input
  </p>
  </blockquote><p>In the figure above, an input is made on frame <code>95</code>, but does not reach the server until frame <code>100</code>. However, both sides continue to simulate the game world during these 5 frames. Once the server receives the input, it needs to rewind the game <em>back</em> to frame <code>95</code> from frame <code>100</code>, and reenact what “actually” have happened.</p><p>Once the reenactment is complete, it sends a snapshot of the entire game world at frame 100. Again, the game world continues forward while the snapshot is in transit.</p><p>Finally, on frame <code>105</code>, the client receives the snapshot reflecting the authoritative state at <code>100</code> and “regenerates” the game world based on it. It must now fast forward up to the current frame in order to reflect what it thinks the server must be seeing at frame <code>105</code>.</p><h5 id="Detailed-Overview">Detailed Overview</h5><p>There is a fair amount of complexity hidden by the previous diagram, especially regarding the way reenactment operates on each side. What <em>actually</em> happens when an input is received by the server, or a snapshot is received by the client? Let’s take a look step-by-step:</p><p><img src="https://s1.gifyu.com/images/extrapolation-overview.gif" alt="Extrapolation overview animation"></p><pre><code>Carousel descriptions:
  ...
  <span className="token keyword">if</span> <span className="token punctuation">(</span><span className="token function">latestSnapshotExists</span><span className="token punctuation">(</span><span className="token punctuation">)</span><span className="token punctuation">)</span> <span className="token punctuation">{</span>

    snapshot <span className="token operator">=</span> <span className="token function">getLatestSnapshot</span><span className="token punctuation">(</span><span className="token punctuation">)</span>
    ...
    <span className="token function">regenerateFromSnapshot</span><span className="token punctuation">(</span>snapshot<span className="token punctuation">)</span>
    <span className="token function">catchUpToCurrentFrameFrom</span><span className="token punctuation">(</span>snapshot<span className="token punctuation">.</span>frame<span className="token punctuation">)</span>

    <span className="token function">deleteLatestSnapshot</span><span className="token punctuation">(</span><span className="token punctuation">)</span> <span className="token comment">// Make room for next snapshot</span>
  ...
  <pre><code className="javascript hljs"><span className="token keyword">function</span> <span className="token function">regenerateFromSnapshot</span><span className="token punctuation">(</span>snapshot<span className="token punctuation">)</span> <span className="token punctuation">{</span>
  snapshot<span className="token punctuation">.</span>chips<span className="token punctuation">.</span><span className="token function">forEach</span><span className="token punctuation">(</span>chipInfo <span className="token operator">=&gt;</span> <span className="token punctuation">{</span>
    ...
    <span className="token keyword">if</span> <span className="token punctuation">(</span><span className="token function">chipDoesNotExist</span><span className="token punctuation">(</span><span className="token punctuation">)</span><span className="token punctuation">)</span> <span className="token punctuation">{</span> <span className="token function">createChip</span><span className="token punctuation">(</span>chipInfo<span className="token punctuation">)</span> <span className="token punctuation">}</span>

    <span className="token comment">// Get a reference to the chip, either newly created or already existing</span>
    chip <span className="token operator">=</span> <span className="token function">getChipById</span><span className="token punctuation">(</span>chipInfo<span className="token punctuation">.</span>id<span className="token punctuation">)</span>

    <span className="token comment">// Adjust physical properties based on snapshot's properties</span>
    <span className="token function">updateChipProperties</span><span className="token punctuation">(</span>chip<span className="token punctuation">,</span> chipInfo<span className="token punctuation">)</span>
  <span className="token punctuation">}</span><span className="token punctuation">)</span>
  <span className="token punctuation">}</span>
  </code></pre>
  ...
  <p><img src="https://s1.gifyu.com/images/extrapolation-popping.gif" alt="Popping" /></p>
  ...
  <p><img src="https://s1.gifyu.com/images/extrapolation-bending.gif" alt="Bending" /></p>
  ...
  </code></pre><p>The result is a smooth path which the eye can follow naturally, but the path may still appear to be “impossible” based on the physics.</p><h6 id="Bending-Implementation">Bending Implementation</h6><p>We can implement bending by snapping the physics to the new position immediately, while slowly updating the rendered position over the course of multiple frames. First we calculate the distance between the new and old object, then we render the old position plus some percentage of the distance.</p><pre><code className="javascript hljs"><span className="token comment">// Determine the number of frames before the position will converge</span>
  ...
  <p><img src="https://media.giphy.com/media/1gXhpaKuQfwJoipAXq/giphy.gif" alt="Estimating Latency" />
